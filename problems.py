...
class ReluRegressionProblem:
    ...
    def loss(self):
        def f(z):
            return (1.0 / self.A.size()[0]) * torch.norm(torch.max(self.A @ (z), torch.zeros(self.A.size()[0])) - self.y, 1)
        return f
...
class MaxAffineRegressionProblem:
    ...
    def loss(self):
        def f(z):
            return (1.0 / self.A.size()[0]) * torch.linalg.norm(torch.max(self.A.mm(z.view(self.βs.size())),dim=1)[0] - self.y, 1)
        return f
...
# Need to discuss this poriton of the code. Why not just give the alt proj part access to self?
    def loss_altproj(problem):
        ...
        def f(z):
            ...
            return torch.norm(z_comp - A.mm(torch.linalg.solve(z_comp, F[0])[0])) + torch.norm(z_comp - y * phase(z_comp))
        return f
...
class QuadraticSensingProblem:
    ...
    def loss(self):
        def f(z):
            ...
            return (1.0 / self.y.size()[0]) * torch.linalg.norm(self.y - torch.sum(self.L.mm(Z) * self.R.mm(Z), dim = 1), 1)
        return f
...
class BilinearSensingProblem:
    ...
    def loss(self):
        def f(z):
            ...
            return (1.0 / self.y.size()[0]) * torch.norm(self.y - torch.sum(self.L.mm(W) * self.R.mm(X), dim = 1), 1)
        return f
    ...
    def initializer(self, δ):
        Δ = torch.randn(self.W.numel() + self.X.numel())
...
def generate_sparse_vector(d, k):
    x = torch.zeros(d)
    x[torch.randperm(d)[:k]] = normalize(torch.randn(k), dim=-1)
    return x
