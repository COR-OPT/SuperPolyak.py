...
def build_bundle_torch(
    ...
    if max_elts is None:
        max_elts = len(y0)
    fvals = np.zeros(max_elts, dtype=np.float64)
    resid = np.zeros(max_elts, dtype=np.float64)
...
def build_bundle_torch_param_groups(
    ...
    """
    params = list(params)
    y0 = parameters_to_vector(params).detach().clone().numpy()
    d = len(y0)
    with torch.enable_grad():
        fy0 = closure().item()
    gap = fy0 - min_f
    ...
    vector_to_parameters(
        torch.from_numpy(y0 - linalg.lstsq(bmtrx[0:1, :], [fvals[0]], rcond=None)[0]),
        params,
    )
    # Compute function gap
    with torch.enable_grad():
    ...
    resid[0] = fy - min_f
    # Exit early if solution escaped ball.
    if np.linalg.norm(parameters_to_vector(params).numpy() - y0) > tau * gap:
        vector_to_parameters(torch.from_numpy(y0), params)
        return params, 1
    # Best solution and function value found so far.
    y_best = parameters_to_vector(params).detach().clone().numpy()
    f_best = resid[0]
    for bundle_idx in range(1, max_elts):
        bmtrx[bundle_idx, :] = _gather_flat_grad(params).numpy()
        # Invariant: resid[bundle_idx - 1] = f(y) - min_f.
        fvals[bundle_idx] = resid[bundle_idx - 1] + bmtrx[bundle_idx, :] @ (
            y0 - parameters_to_vector(params).numpy()
        )
        if linsys_solver is BundleLinearSystemSolver.NAIVE:
            dy = np.linalg.lstsq(
                bmtrx[0 : (bundle_idx + 1), :], fvals[0 : (bundle_idx + 1)], rcond=None
            ...
            dy, istop, itn = lsmr(
                ...
                btol=max(1e-15, gap),
                conlim=0.0,
                x0=parameters_to_vector(params).numpy() - y0,
            )[:3]
        else:
            raise ValueError(f"Unrecognized linear system solver {linsys_solver}!")
        # Update point and function gap.
        vector_to_parameters(torch.from_numpy(y0 - dy), params)
        with torch.enable_grad():
            fy = closure().item()
        resid[bundle_idx] = fy - min_f
        # Terminate early if new point escaped ball around y₀.
        if np.linalg.norm(dy) > tau * gap:
            vector_to_parameters(torch.from_numpy(y_best), params)
